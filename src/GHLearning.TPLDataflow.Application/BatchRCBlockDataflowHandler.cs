	...
	ITelegramService telegramService) : IDataflowHandler, IDisposable, IAsyncDisposable
{
	...
	private async Task PrepareAsync(CancellationToken cancellationToken = default)
	{
		...
		try
		{
			if (_messageSubject is not null)
				return;
			...
			_messageSubject = new Subject<DataflowMessage?>();
			_messageSubject
			   .Buffer(TimeSpan.FromSeconds(30), 10)  // 每 30 秒或累積 10 條訊息，就進行批次處理
			   .Where(batch => batch.Any())  // 只處理有訊息的批次
			   .Do(batch => hidePhoneNumberBlock.Post(item: [.. batch!]))
			   .Subscribe();
		}
		finally
		{
